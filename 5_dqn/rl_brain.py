...
class DeepQNetwork:
    def __init__(
        ...
        # consist of [target, evaluate_target]
        self._build_net()
        ...
        self.replace_target_op = [
            tf.assign(target, e) for target, e in zip(target_params, eval_params)]
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('target_net'):
            e1 = tf.layers.dense(
                self.s, 20, tf.nn.relu, kernel_initializer=w_initializer, bias_initializer=b_initializer)
            self.q_next = tf.layers.dense(
                e1, self.n_actions, kernel_initializer=w_initializer, bias_initializer=b_initializer)
